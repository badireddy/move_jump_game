...
// All 50 US states with their capitals and USPS codes. State names match the
// labels in us-atlas (states-10m), so no `mapName` aliases are needed.
const raw: Omit<StateItem, 'id'>[] = [
...
export const STATES: StateItem[] = raw.map((s) => ({ ...s, id: `us:${s.code}` }))

export const STATE_BY_ID: Record<string, StateItem> = Object.fromEntries(
  STATES.map((s) => [s.id, s]),
)
...
const NAME_INDEX: Record<string, StateItem> = (() => {
  const idx: Record<string, StateItem> = {}
  for (const s of STATES) {
    idx[normalizeName(s.name)] = s
    if (s.mapName) idx[normalizeName(s.mapName)] = s
  }
  return idx
})()

export function stateByMapName(name: string): StateItem | undefined {
